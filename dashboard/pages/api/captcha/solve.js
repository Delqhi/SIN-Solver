...
export default async function handler(req, res) {
  ...
  try {
    ...
    logger.info('CAPTCHA_SOLVE', `[${req.method}] ${req.url}`);
    ...
    // 5. Validate input
    const validation = validateInput(req.body, ['captcha_data', 'image_url']);
    if (!validation.valid) {
    ...
    // 7. Success response
    logger.info('CAPTCHA_SOLVE', '[SUCCESS] 200 OK');
    return res.status(200).json(result);

  } catch (error) {
    // 8. Error handling
    logger.error('CAPTCHA_SOLVE', error.message);
