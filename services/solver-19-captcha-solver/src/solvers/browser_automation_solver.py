...
class ElementDetector:
    ...
    def __init__(self, model_path: str = "/Users/jeremy/dev/sin-solver/models/yolov8x.pt", confidence_threshold: float = 0.5):
        ...
        try:
            ...
            logger.error(f"❌ Failed to initialize YOLOv8: {e}. YOLOv8 is required for production operation.")
...
class BrowserController:
    ...
    def __init__(
        self,
        cdp_url: str = "http://agent-05-steel-browser:3005",
        timeout: float = 30.0
    ):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 200 or response.status_code == 201:
                    ...
                    logger.error(f"Failed to create Steel session: {response.status_code} - {response.text}")
                    return False
        except Exception as e:
...
class BrowserAutomationSolver:
    ...
    async def solve_with_vision(
        ...
        try:
            ...
            if not captcha_elements:
                logger.warning("No captcha elements found via heuristics, using all elements for planning.")
            ...
            return {
                ...
                "planned_actions": [
                    {"type": a.type, "target": a.target, "reason": a.reason, "coords": self._get_action_coords(a, elements)}
                    for a in actions
                ],
                "solver": "vision_only"
            }
    ...
    def _get_action_coords(self, action: Action, elements: List[Element]) -> Optional[Tuple[int, int]]:
