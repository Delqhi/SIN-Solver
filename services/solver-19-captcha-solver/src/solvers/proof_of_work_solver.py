...
class ProofOfWorkSolver:
    ...
    def solve_sync(self, challenge: PoWChallenge, timeout: float = 30.0) -> Dict[str, Any]:
        ...
        logger.info(f"🔍 Solving PoW challenge (difficulty={challenge.difficulty}, max={challenge.max_iterations})")
        ...
        while number < challenge.max_iterations:
            ...
            if self._check_solution(challenge, number):
                ...
                logger.info(f"✅ PoW solved! number={number}, took={took} iterations, time={elapsed_ms:.2f}ms")
        ...
        elapsed_ms = (time() - start_time) * 1000
        logger.error(f"❌ PoW solve failed: exhausted {challenge.max_iterations} iterations in {elapsed_ms:.2f}ms")
    ...
    async def solve_batch(
        self,
        challenges: list[PoWChallenge],
        timeout_per_challenge: float = 30.0
    ) -> list[Dict[str, Any]]:
        """
        ...
        tasks = [
            solve_with_error_handling(challenge)
...
class AltchaDetector:
    ...
    def extract_challenge(self, html_content: str) -> Optional[PoWChallenge]:
        ...
        if 'challenge' in extracted and 'salt' in extracted:
            ...
            logger.info(f"🔍 Extracted ALTCHA challenge from HTML (difficulty={challenge_data['difficulty']})")
...
async def solve_altcha_challenge(challenge_dict: Dict[str, Any], timeout: float = 30.0) -> Dict[str, Any]:
    """
