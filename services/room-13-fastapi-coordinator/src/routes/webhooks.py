...
redis_client = redis.Redis(
    host=REDIS_HOST, port=REDIS_PORT, db=REDIS_DB_WEBHOOKS, decode_responses=True
)
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            response = await asyncio.wait_for(
                client.post(webhook["url"], content=payload_str, headers=headers, timeout=30.0),
                timeout=35.0,
            )
...
async def list_webhooks(
    ...
    return {"total": total, "skip": skip, "limit": limit, "webhooks": webhooks}
...
async def get_webhook_deliveries(
    ...
    return {"total": total, "skip": skip, "limit": limit, "deliveries": deliveries}
