    ...
    try:
        async with await get_captcha_worker_client() as client:
            ...
            if response.status_code != 200:
                ...
                raise HTTPException(
                    status_code=response.status_code, detail="Captcha solver failed"
                )
        ...
        raise HTTPException(status_code=503, detail="Captcha solver service unavailable")
...
async def solve_text_captcha(image_base64: str, timeout: int = 30):
    """
    ...
    request = SolveRequest(
        captcha_type=CaptchaType.TEXT, image_base64=image_base64, timeout=timeout
    )
...
async def solve_image_grid_captcha(
    image_base64: str, instructions: str, grid_size: str = "3x3", timeout: int = 45
):
    """
...
async def solve_with_browser(
    url: str, captcha_type: CaptchaType = CaptchaType.AUTO, timeout: int = 60
):
    """
    ...
    Args:
        ...
        timeout: Maximum time to wait (seconds)
    """
    request = SolveRequest(captcha_type=captcha_type, url=url, use_browser=True, timeout=timeout)
    return await solve_captcha(request)
    ...
    try:
        async with await get_captcha_worker_client() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(
                    status_code=response.status_code, detail="Failed to get solver status"
                )
    ...
    try:
        async with await get_captcha_worker_client() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(
                    status_code=response.status_code, detail="Failed to get rate limits"
                )
    ...
    try:
        async with await get_captcha_worker_client() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(
                    status_code=response.status_code, detail="Failed to get statistics"
                )
...
async def batch_solve_captchas(requests: List[SolveRequest], background_tasks: BackgroundTasks):
    """
