        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await asyncio.wait_for(
                    client.post(full_url, json=request_body, headers=headers, timeout=30.0),
                    timeout=35.0,
                )
                ...
                await registry.update_service_stats(
                    service_name, response.status_code < 500, response_time
                )
...
async def gateway_service_health(registry=None):
    ...
    try:
        ...
        return {"timestamp": datetime.utcnow().isoformat(), "services": detailed_health}
    except Exception as e:
...
async def gateway_metrics(registry=None):
    ...
    try:
        ...
        avg_response_time = (
            sum(s.get("avg_response_time_ms", 0) for s in services) / len(services)
            if services
            else 0
        )
...
async def manual_health_check(service_name: str, registry=None):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await asyncio.wait_for(
                    client.get(health_endpoint, timeout=10.0), timeout=12.0
                )
