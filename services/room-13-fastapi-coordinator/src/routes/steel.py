    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code >= 400:
                ...
                raise HTTPException(
                    status_code=response.status_code, detail=f"Steel API error: {response.text}"
                )
...
async def steel_status():
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await asyncio.wait_for(
                client.get(f"{STEEL_BASE_URL}/health", timeout=10.0), timeout=15.0
            )
...
async def create_steel_session(
    ...
    session_config = {"stealth": stealth, "options": {}}
...
async def take_steel_screenshot(
    session_id: str, full_page: bool = False, selector: Optional[str] = None
):
    ...
    result = await steel_request(
        "POST", f"/v1/sessions/{session_id}/screenshot", data, timeout=30.0
    )
    return result


@router.post("/sessions/{session_id}/pdf")
async def generate_steel_pdf(session_id: str, format: str = "A4", print_background: bool = True):
...
async def evaluate_steel_script(session_id: str, script: str = Body(..., embed=True)):
    """Execute JavaScript in Steel Browser session"""
    result = await steel_request(
        "POST", f"/v1/sessions/{session_id}/evaluate", {"script": script}, timeout=30.0
    )
    return result
...
async def click_steel_element(session_id: str, selector: str = Body(..., embed=True)):
    """Click element in Steel Browser session"""
    result = await steel_request(
        "POST", f"/v1/sessions/{session_id}/click", {"selector": selector}, timeout=30.0
    )
    return result
...
async def select_steel_option(session_id: str, selector: str = Body(...), value: str = Body(...)):
...
async def wait_steel_selector(
    session_id: str, selector: str = Body(..., embed=True), timeout_ms: int = Body(30000)
):
...
async def set_steel_cookies(session_id: str, cookies: List[Dict[str, Any]] = Body(...)):
    """Set cookies in Steel Browser session"""
    result = await steel_request("POST", f"/v1/sessions/{session_id}/cookies", {"cookies": cookies})
    return result
