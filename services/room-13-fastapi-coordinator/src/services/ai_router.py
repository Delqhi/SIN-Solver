...
class FreeAIRouter:
    ...
    async def route_text(self, prompt: str, system: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            "success": False,
            "error": "All text providers failed or exhausted",
            "provider": None,
        }

    async def route_vision(
        self, image_base64: str, prompt: str, system: Optional[str] = None
    ) -> Dict[str, Any]:
        self._check_daily_reset()
        ...
        return {
            "success": False,
            "error": "All vision providers failed or exhausted",
            "provider": None,
        }

    async def _call_text_provider(
        self, name: str, prompt: str, system: Optional[str]
    ) -> Dict[str, Any]:
    ...
    async def _call_vision_provider(
        self, name: str, image_base64: str, prompt: str, system: Optional[str]
    ) -> Dict[str, Any]:
        provider = self.providers[name]
            ...
            async with httpx.AsyncClient(timeout=90.0) as client:
                if name == "gemini":
                    return await self._call_gemini_vision(
                        client, provider, image_base64, prompt, system
                    )
                elif name == "groq":
                    return await self._call_groq_vision(
                        client, provider, image_base64, prompt, system
                    )
