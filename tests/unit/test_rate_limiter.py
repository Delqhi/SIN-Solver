...
class TestRateLimiter:
    ...
    async def test_initial_request_allowed(self, rate_limiter):
        ...
        is_limited, count = await rate_limiter.is_rate_limited(
            "client_1", max_requests=10, window_seconds=60
        )
    ...
    async def test_separate_clients_tracked_separately(self, rate_limiter):
        ...
        is_limited_1, count_1 = await rate_limiter.is_rate_limited(
            "client_1", max_requests=10, window_seconds=60
        )
        is_limited_2, count_2 = await rate_limiter.is_rate_limited(
            "client_2", max_requests=10, window_seconds=60
        )
...
class TestRateLimiterEdgeCases:
    ...
    async def test_zero_max_requests(self, rate_limiter):
        ...
        is_limited, count = await rate_limiter.is_rate_limited(
            "client_1", max_requests=0, window_seconds=60
        )
        assert is_limited is True
    ...
    async def test_negative_max_requests(self, rate_limiter):
        ...
        is_limited, count = await rate_limiter.is_rate_limited(
            "client_1", max_requests=-1, window_seconds=60
        )
