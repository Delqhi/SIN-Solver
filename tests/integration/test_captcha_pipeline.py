...
class TestCaptchaPipeline:
    ...
    async def test_text_captcha_flow(self):
        ...
        request = CaptchaSolveRequest(
            image_data=image_b64, captcha_type="text", client_id="test_pipeline"
        )

        with (
            patch("src.main.veto_engine") as mock_veto,
    ...
    async def test_rate_limited_request(self):
        ...
        request = CaptchaSolveRequest(
            image_data=self.create_test_captcha(), client_id="rate_limited_client"
        )
    ...
    async def test_invalid_image_data(self):
        ...
        request = CaptchaSolveRequest(image_data="invalid_base64_data!!!", client_id="test_client")
    ...
    async def test_solver_not_initialized(self):
        ...
        request = CaptchaSolveRequest(
            image_data=self.create_test_captcha(), client_id="test_client"
        )
    ...
    async def test_batch_processing(self):
        ...
        batch_request = BatchCaptchaRequest(
            ...
            requests=[
                CaptchaSolveRequest(
                    image_data=self.create_test_captcha("TEST1"), client_id="batch_test"
                ),
                CaptchaSolveRequest(
                    image_data=self.create_test_captcha("TEST2"), client_id="batch_test"
                ),
            ],
        )
