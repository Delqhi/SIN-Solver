...
CONTAINERS = {
    ...
    "builder-1.1-captcha-worker": {"url": "http://localhost:8019/health", "type": "http"},
            ...
            try:
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                    if resp.status == 200:
                        ...
                        print(
                            f"{'✅' if has_captcha_metric else '⚠️ '} {name}: Metrics {'OK' if has_captcha_metric else 'incomplete'}"
                        )
                    else:
...
async def test_log_output():
    ...
    for container in containers:
        result = subprocess.run(
            ["docker", "logs", "--tail", "5", container], capture_output=True, text=True
        )
