...
class CaptchaBenchmark:
    ...
    def run_benchmark(self, images_by_type: Dict[str, List[Path]]) -> bool:
        ...
        for captcha_type in CAPTCHA_TYPES:
            ...
            print(f"   ✅ {captcha_type}: {type_accuracy:.1f}% accuracy ({type_correct}/{len(images)})")
    ...
    def generate_text_report(self) -> str:
        ...
        weak_types = [t for t in sorted_types if self.results["per_type_metrics"][t]["accuracy"] < 90.0]
        if weak_types:
        ...
        excellent = [t for t in CAPTCHA_TYPES if self.results["per_type_metrics"][t]["accuracy"] >= 95.0]
        good = [t for t in CAPTCHA_TYPES if 90.0 <= self.results["per_type_metrics"][t]["accuracy"] < 95.0]
        needs_work = [t for t in CAPTCHA_TYPES if self.results["per_type_metrics"][t]["accuracy"] < 90.0]
    ...
    def generate_ascii_charts(self) -> str:
        ...
        if times:
            ...
            bucket_labels = ["0-5ms", "5-10ms", "10-20ms", "20-50ms", "50-100ms", 
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if weak_types:
            ...
            for captcha_type, accuracy in sorted(weak_types, key=lambda x: x[1]):
                display_name = TYPE_DISPLAY_NAMES.get(captcha_type, captcha_type)
                recommendations.append(f"   • {display_name}: Currently {accuracy:.1f}% - needs more training data")
