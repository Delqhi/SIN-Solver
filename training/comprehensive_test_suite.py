...
class TestDatasetStructure:
    ...
    def test_all_captcha_types_have_base_and_augmented_images(self):
        ...
        for captcha_type in CAPTCHA_TYPES:
            ...
            assert len(images) == 44, f"{captcha_type} has {len(images)} images, expected 44 (4 base + 40 aug)"
...
class TestImageFormat:
    ...
    def test_all_images_have_standard_resolution(self):
        for captcha_type in CAPTCHA_TYPES:
            ...
            for img_path in captcha_dir.glob("*.png"):
                img = Image.open(img_path)
                assert img.size == (400, 300), f"{img_path.name} has size {img.size}, expected (400, 300)"
    ...
    def test_image_file_sizes_reasonable(self, captcha_type):
        ...
        for img_path in captcha_dir.glob("*.png"):
            ...
            assert 1024 < file_size < 1000000, f"{img_path.name} size {file_size} bytes is unreasonable"
    ...
    def test_image_aspect_ratio_consistency(self):
        for captcha_type in CAPTCHA_TYPES:
            ...
            for img_path in captcha_dir.glob("*.png"):
                ...
                assert abs(aspect_ratio - expected_ratio) < 0.01, f"Aspect ratio mismatch in {img_path.name}"
...
class TestImageQuality:
    ...
    def test_image_not_blank(self, captcha_type):
        ...
        for img_path in captcha_dir.glob("*.png"):
            ...
            assert max_pixel - min_pixel > 10, f"{img_path.name} appears to be blank or nearly blank"
...
class TestTrainingIntegration:
    ...
    def test_generate_training_statistics_report(self):
        ...
        report["splits"]["train"]["percentage"] = (report["splits"]["train"]["count"] / total_images * 100)
        report["splits"]["val"]["percentage"] = (report["splits"]["val"]["count"] / total_images * 100)
        ...
        print(f"Training Set: {report['splits']['train']['count']} ({report['splits']['train']['percentage']:.1f}%)")
        print(f"Validation Set: {report['splits']['val']['count']} ({report['splits']['val']['percentage']:.1f}%)")
        print(f"{'='*80}")
