...
async def solve_captcha(request: CaptchaSolveRequest):
    ...
    # Rate limiting check
    if rate_limiter:
        is_limited, current = await rate_limiter.is_rate_limited(request.client_id, max_requests=20, window_seconds=60)
        if is_limited:
    ...
    try:
        with CAPTCHA_SOLVE_DURATION.time():
            ...
            if request.image_data:
                ...
                result = await veto_engine.solve_with_browser(request.url, captcha_type="auto", timeout=request.timeout)
