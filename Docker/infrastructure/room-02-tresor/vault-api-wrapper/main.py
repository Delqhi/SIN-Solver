    ...
    async def is_healthy(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.addr}/v1/sys/health", timeout=5.0)
    ...
    async def list_secrets(self, path: str = "") -> List[str]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.request("LIST", url, headers=self.headers, timeout=10.0)
...
vault_client = VaultClient(
    addr=settings.VAULT_ADDR, token=settings.VAULT_TOKEN, mount_path=settings.VAULT_MOUNT_PATH
)
...
async def get_secret(path: str):
    ...
    return SecretResponse(path=path, data=result["data"], metadata=result.get("metadata"))
...
async def create_secret(path: str, secret: SecretData):
    ...
    return SecretResponse(path=path, data=secret.data, metadata=metadata)
...
async def list_secrets(prefix: str = Query("", description="Path prefix to list")):
    ...
    return SecretListResponse(paths=secrets, count=len(secrets))
...
async def sync_to_vercel(
    ...
    secret_mappings = {
        ...
        "redis": {"prefix": "REDIS_", "keys": ["host", "port", "password"]},
        "opencode": {"prefix": "OPENCODE_", "keys": ["api_key", "base_url"]},
        "vercel": {
            "prefix": "",
            "keys": [
                "NEXT_PUBLIC_API_URL",
                "NEXT_PUBLIC_CODESERVER_API_URL",
...
async def sync_to_n8n(background_tasks: BackgroundTasks):
    ...
    - OpenCode API
    """
    n8n_sync = N8nSync(host=settings.N8N_HOST, api_key=settings.N8N_API_KEY)
...
async def sync_to_all(background_tasks: BackgroundTasks):
    ...
    return {"vercel": vercel_result, "n8n": n8n_result}
...
async def initialize_secrets():
    ...
    secrets_to_init = {
        ...
        "github": {"token": os.getenv("GITHUB_TOKEN", ""), "repo": os.getenv("GITHUB_REPO", "")},
        "codeserver": {
...
if __name__ == "__main__":
    ...
    uvicorn.run("main:app", host="0.0.0.0", port=8201, reload=True, log_level="info")
