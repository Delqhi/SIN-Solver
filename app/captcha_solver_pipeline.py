...
class TesseractOCREngine:
    ...
    def extract_text(self, image_path: str) -> OCRResult:
        ...
        try:
            ...
            logger.info(f"Tesseract: '{text.strip()}' (conf: {confidence:.2f}, time: {elapsed:.2f}s)")
    ...
    def extract_text_preprocessed(self, image_path: str) -> OCRResult:
        ...
        try:
            ...
            logger.info(f"Tesseract (preprocessed): '{text.strip()}' (conf: {confidence:.2f}, time: {elapsed:.2f}s)")
            return OCRResult(text.strip(), confidence, "tesseract", elapsed)
    ...
    def extract_text(self, image_path: str) -> OCRResult:
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"PaddleOCR: '{final_text}' (conf: {avg_confidence:.2f}, time: {elapsed:.2f}s)")
...
class CaptchaSolverPipeline:
    ...
    def vote_ocr_results(self, results: List[OCRResult], weights: Dict[str, float] = None) -> OCRResult:
        ...
        logger.info(f"Consensus: '{winning_text_str}' ({len(winning_votes)} votes, conf: {final_conf:.2f})")
