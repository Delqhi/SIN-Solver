...
class GitLabService:
    ...
    async def _ensure_project(self) -> bool:
        ...
        try:
            ...
            resp = requests.get(f"{self.api_url}/projects?search={self.base_project_name}", headers=self.headers)
            projects = resp.json()
            ...
            resp = requests.post(f"{self.api_url}/projects", headers=self.headers, json=project_data)
    ...
    async def upload_screenshot(self, file_path: str, commit_message: str = "Upload screenshot") -> Optional[str]:
        """
        ...
        try:
            ...
            if resp.status_code == 201:
                ...
                raw_url = f"https://gitlab.com/{self.current_project_path}/-/raw/main/{unique_filename}"
