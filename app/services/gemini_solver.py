            ...
            try:
                ...
                response = await self.client.aio.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        temperature=0.0, max_output_tokens=200 if high_tier else 100
                    ),
                )
                ...
                logger.info(
                    f"Gemini ({model}) solved CAPTCHA in {time_ms}ms: '{solution}' (confidence: {confidence})"
                )
                ...
                if "rate limit" in error_str or "429" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"⚠️ Gemini Rate Limit hit. Retrying in {sleep_time:.1f}s... (Attempt {attempt + 1}/{max_retries})"
                        )
