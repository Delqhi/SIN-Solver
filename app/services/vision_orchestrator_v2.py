...
def parse_gemini_response_flexible(solution: str, target_object: str) -> List[Dict[str, Any]]:
    ...
    for idx, pattern in enumerate(json_patterns):
        ...
        for match in matches:
            try:
                ...
                if objects:
                    logger.info(f"✅ Strategy 2.{idx+1} SUCCESS: Extracted {len(objects)} objects")
                    return objects
            except:
                continue
    ...
    for idx, pattern in enumerate(box_patterns):
        ...
        if matches:
            ...
            if objects:
                logger.info(f"✅ Strategy 3.{idx+1} SUCCESS: Extracted {len(objects)} boxes from coordinates")
                return objects
    ...
    for pattern in positive_patterns:
        ...
        if match:
            try:
                ...
                if count_str:
                    count = int(count_str)
                    logger.info(f"🎯 Strategy 4 SUCCESS: NLP detected {count} objects mentioned in text")
    ...
    positive_keywords = ['yes', 'found', 'detected', 'present', 'visible', 'located', 'identified', 'there is', 'there are']
