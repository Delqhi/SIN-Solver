...
class AdvancedSolver:
    ...
    async def solve_click_order(self, image_bytes: bytes, instruction: str) -> List[Dict[str, float]]:
        """
    ...
    async def solve_rotate(self, image_bytes: bytes) -> float:
        ...
        try:
            ...
            if not json_match:
                ...
                offset_match = re.search(r'offset_percentage["\']?\s*[:=]\s*(\d+\.?\d*)', cleaned, re.IGNORECASE)
                if offset_match:
