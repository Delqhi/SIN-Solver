            ...
            try:
                for site, min_size in self.pool_config.items():
                    ...
                    if current_size < min_size:
                        logger.info(
                            f"📉 [TokenPool] {site} low ({current_size}/{min_size}). Scheduling solve..."
                        )
        ...
        for site, count in demand.items():
            ...
            if count > 10:
                ...
                if self.dynamic_config.get(site) != new_min:
                    logger.info(
                        f"📈 [PredictivePool] Ramping up {site} pool to {new_min} due to high demand"
                    )
            ...
            try:
                ...
                for site, min_size in self.dynamic_config.items():
                    ...
                    if current_size < min_size:
                        ...
                        logger.info(
                            f"📉 [TokenPool] {site} low ({current_size}/{min_size}). Pre-solving {needed} tokens..."
                        )
