...
class YOLOSolver:
    ...
    def _load_model(self):
        try:
            ...
            if os.path.exists(self.model_path):
                ...
                logger.warning(
                    f"⚠️ YOLO model not found at {self.model_path}. Detection will be disabled."
                )
        except ImportError:
            logger.error("❌ ultralytics not installed. YOLO detection disabled.")

    def detect_objects(
        self, image_path: str, target_classes: List[str] = None, confidence_threshold: float = 0.25
    ) -> List[Dict[str, Any]]:
        if not self.model:
            return []
        ...
        try:
            ...
            results = self.model(
                image_path, conf=confidence_threshold, iou=0.45, imgsz=640, verbose=False
            )
            detections = []
                ...
                for box in boxes:
                    ...
                    if target_classes:
                        ...
                        for target in target_classes:
                            ...
                            if normalized_label == t_low or t_low in synonyms.get(
                                normalized_label, []
                            ):
                                match_found = True
                                break
            ...
            logger.info(
                f"🎯 YOLO detected {len(detections)} objects with confidence > {confidence_threshold}"
            )
            return detections
    ...
    def detect_grid(self, image_path: str, target_classes: List[str] = None) -> List[bool]:
        ...
        try:
            ...
            if img is None:
                logger.error(
                    f"❌ YOLO could not read image at {image_path}. Check permissions or file integrity."
                )
                return grid_results
