            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                with open(file_path, "rb") as audio_file:
                    ...
                    if resp.status_code == 200:
                        ...
                        logger.warning(f"Groq Whisper failed ({resp.status_code}), falling back to Google")
...
async def try_solve_audio_challenge(page: Any) -> bool:
    ...
    """
    try:
        audio_btn = await page.query_selector('#recaptcha-audio-button, button[title*="audio"], .rc-audio-challenge-button')
        if not audio_btn:
            return False
        ...
        download_link = await page.query_selector('.rc-audiochallenge-download-link, a[href*=".mp3"]')
