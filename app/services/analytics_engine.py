...
class AnalyticsEngine:
    ...
    async def save_to_persistence(self):
        ...
        await redis.set_json("analytics:history", self.solve_history[-100:]) # Persist last 100 for recent stats
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        success_rate = (
            (self.metrics.successful_solves / max(1, self.metrics.total_solves)) * 100
        )
        
        avg_solve_time = (
            self.metrics.total_solve_time_ms / max(1, self.metrics.total_solves)
        )
        
        cache_hit_rate = (
            (self.metrics.cache_hits / max(1, self.metrics.total_solves)) * 100
        )
        
        revenue_per_hour = (
            self.metrics.total_revenue / max(0.01, session_duration_hours)
        )
        
        # CAPTCHAs per hour
        captchas_per_hour = (
            self.metrics.total_solves / max(0.01, session_duration_hours)
        )
