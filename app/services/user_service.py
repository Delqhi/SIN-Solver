...
class UserService:
    ...
    async def create_secondary_api_key(self, db: AsyncSession, user_id: str, key_data: APIKeyCreate) -> str:
    ...
    async def update_usage_stats(self, db: AsyncSession, user_id: str, usage_increment: int = 1, cost: float = 0.0):
    ...
    async def get_user_stats(self, db: AsyncSession, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            captcha_stmt = select(
                ...
                and_(
                    CaptchaSolution.user_id == user_id,
                    CaptchaSolution.created_at >= since_date
                )
            )
            ...
            api_stmt = select(
                ...
                func.count(func.case([(APILog.status_code >= 400, 1)], else_=0)).label('error_count')
            ).where(
                and_(
            ...
            usage_percentage = (user.monthly_used / user.monthly_limit) * 100 if user.monthly_limit > 0 else 0
            ...
            return {
                ...
                "api_stats": {
                    ...
                    "success_rate": round(
                        (api_stats.total_requests - api_stats.error_count) / max(1, api_stats.total_requests) * 100,
                        2
                    ) if api_stats.total_requests else 100
                }
            }
    ...
    async def get_system_stats(self, db: AsyncSession, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            user_stmt = select(
                ...
                func.count(func.case([(User.is_verified == True, 1)], else_=0)).label('verified_users'),
                func.count(func.case([(User.tier == UserTier.FREE, 1)], else_=0)).label('free_users'),
                func.count(func.case([(User.tier == UserTier.PRO, 1)], else_=0)).label('pro_users'),
                func.count(func.case([(User.tier == UserTier.BUSINESS, 1)], else_=0)).label('business_users')
            ...
            total_user_stmt = select(
                ...
                func.count(func.case([(User.is_active == True, 1)], else_=0)).label('active_users_all_time')
            )
            ...
            captcha_stmt = select(
                ...
                func.avg(CaptchaSolution.solving_time_ms).label('avg_solve_time'),
                func.sum(CaptchaSolution.cost).label('total_cost'),
                func.count(func.case([(CaptchaSolution.solver_used == 'gemini', 1)], else_=0)).label('gemini_uses'),
                func.count(func.case([(CaptchaSolution.solver_used == 'mistral', 1)], else_=0)).label('mistral_uses'),
                func.count(func.case([(CaptchaSolution.solver_used == 'groq', 1)], else_=0)).label('groq_uses')
            ...
            return {
                ...
                "revenue": {
                    ...
                    "per_user_average": round(monthly_recurring_revenue / max(1, total_user_stats.active_users_all_time or 1), 2)
                },
                "stats": self.stats
            }
