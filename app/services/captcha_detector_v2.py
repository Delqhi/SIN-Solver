...
HEALTH_CHECK_STATUS = Gauge(
    "health_check_status", "Health check status (1=healthy, 0=unhealthy)", ["check_type"]
)
...
class RateLimiter:
    ...
    async def is_allowed(self, client_id: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            tokens = min(
                self.burst_size, tokens + (time_passed * self.max_requests / self.window_seconds)
            )
...
class AsyncQueueManager:
    ...
    async def enqueue(self, request: CaptchaSolveRequest, batch_id: Optional[str] = None) -> str:
        """
...
class ZoneAnalyzer:
    ...
    async def analyze_zone(
        self, zone_image: np.ndarray, zone_id: int, timeout_ms: int = 3000
    ) -> ZoneAnalysis:
        """
        ...
        try:
            gemini_result, mistral_result = await asyncio.gather(
                gemini_task, mistral_task, return_exceptions=True
            )
        except Exception as e:
...
Return JSON."""
    ...
    def _merge_zone_results(
        self, gemini_result: Dict[str, Any], mistral_result: Dict[str, Any]
        ...
        if gemini_type == mistral_type:
            confidence = (
                gemini_result.get("confidence", 0.5) + mistral_result.get("confidence", 0.5)
            ) / 2
...
class UniversalCaptchaDetectorV2:
    ...
    def __init__(
        ...
        self.redis = async_redis.Redis(host=redis_host, port=redis_port, decode_responses=True)
    ...
    async def detect(
        self, screenshot: np.ndarray, timeout_ms: int = 3000, batch_id: Optional[str] = None
    ...
    async def solve_request(self, request: CaptchaSolveRequest) -> CaptchaSolveResponse:
        """
        ...
        try:
            ...
            # Detect CAPTCHA
            result = await self.detect(image, timeout_ms=request.timeout * 1000)
    ...
    async def solve_batch(self, batch_request: BatchCaptchaRequest) -> List[CaptchaSolveResponse]:
        """
    ...
    async def enqueue_solve(
        self, request: CaptchaSolveRequest, batch_id: Optional[str] = None
    ) -> str:
        """
    ...
    async def _analyze_zones(self, screenshot: np.ndarray, timeout_ms: int) -> List[ZoneAnalysis]:
        ...
        for zone_id in range(9):
            ...
            task = self.zone_analyzer.analyze_zone(zone_image, zone_id, timeout_ms // 9)
            tasks.append(task)
    ...
    def _detect_blockers(self, screenshot: np.ndarray, zones: List[ZoneAnalysis]) -> BlockerType:
    ...
    def _determine_captcha_type(self, zones: List[ZoneAnalysis]) -> Tuple[CaptchaType, float]:
    ...
    def _choose_fallback_strategy(
        self, captcha_type: CaptchaType, blocker: BlockerType, confidence: float
    ) -> str:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        # Overall status
        all_healthy = all(c["status"] == "healthy" for c in checks["checks"].values())
...
async def solve_async(request: CaptchaSolveRequest, batch_id: Optional[str] = None):
    ...
    return {"job_id": job_id, "status": "queued", "check_url": f"/api/solve/async/{job_id}"}
...
async def get_async_result(job_id: str):
    ...
    if result:
        ...
        return {"job_id": job_id, "status": "processing", "message": "Job not yet complete"}
