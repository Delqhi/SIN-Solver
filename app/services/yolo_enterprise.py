...
class YOLOEnterpriseService:
    ...
    async def load_model(self):
        ...
        with self._model_lock:
            ...
            try:
                ...
                logger.info("YOLO model loaded successfully", classes=len(self.CLASSES))
    ...
    async def predict(self, image_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await loop.run_in_executor(None, self._sync_predict, image_data)

            # Record metrics
            inference_time = int((time.time() - start_time) * 1000)
            metrics.histogram("yolo_inference_time_ms", inference_time)
            metrics.increment("yolo_predictions", tags={"class": result["class"]})
    ...
    def _sync_predict(self, image_data: bytes) -> Dict[str, Any]:
        ...
        for i, conf in enumerate(probs.data.tolist()):
            all_predictions.append({"class": self.CLASSES[i], "confidence": float(conf)})
