...
def extract_click_coordinates_from_response(solution: str) -> List[Dict[str, float]]:
    ...
    for pattern in coord_patterns:
        ...
        for match in matches:
            try:
                coordinates.append({"x": float(match[0]), "y": float(match[1])})
            except:
                continue
    return coordinates
...
class ViPerStrikeProcessor:
    ...
    async def process(orchestrator, image_path: str, task_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            target_boxes = [
                world_model[i].get("box_2d") for i in mapping["targets"] if i < len(world_model)
            ]
            return {
                "success": True,
...
class VisionOrchestrator:
    ...
    async def solve_visual_task(
        self, image_path: str, prompt: str, complexity: str = "auto"
    ) -> Dict[str, Any]:
        ...
        cache_key = hashlib.md5(
            f"{self.cache.get_image_hash(image_path)}:{prompt}".encode()
        ).hexdigest()
        ...
        if cached:
            ...
            return (
                ...
                else {"solution": cached, "confidence": 1.0}
            )
        ...
        if complexity == "high" or "select all" in prompt.lower():
            ...
            res = await self._run_with_backoff(
                "ViPerStrike", ViPerStrikeProcessor.process(self, image_path, prompt)
            )
            if res.get("success"):
        ...
        if yolo_res and ("slider" in prompt.lower() or "offset" in prompt.lower()):
            ...
            if piece and gap:
                ...
                return {
                    "solution": json.dumps({"offset_percentage": offset}),
                    "confidence": 1.0,
                    "model": "local-yolo-sim",
                }
        ...
        tasks = [
            run_with_retry_and_backoff(
                "Gemini", lambda: self.gemini_solver_engine.solve_image_path(image_path, prompt)
            ),
            run_with_retry_and_backoff(
                "Mistral", lambda: self._mistral_solve_path(image_path, prompt)
            ),
            run_with_retry_and_backoff(
                "Groq", lambda: self.groq_solver_engine.solve_image(image_path, prompt)
            ),
        ]
        ...
        if results:
            results.sort(
                key=lambda x: (x.get("confidence", 0), -x.get("time_ms", 9999)), reverse=True
            )
            res = results[0]
        else:
            res = {
                "success": False,
                "error": "All solvers failed in parallel race (Backoff exhausted)",
            }
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_rate_limit and attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"🛑 {name} rate limited (Attempt {attempt + 1}). Backoff: {wait_time:.2f}s"
                    )
...
class CaptchaVisionSolver:
    ...
    async def solve_with_visual_grounding(
        self, image_path: str, target_object: str
    ) -> List[Dict[str, Any]]:
        ...
        return [
            {"box_2d": obj["box_2d"]}
            for obj in parse_gemini_response_flexible(str(solution), target_object)
            if "box_2d" in obj
        ]

    def transform_bounding_boxes_to_clicks(
        self, bounding_boxes: List[Dict[str, Any]], container_box: Dict[str, float]
    ) -> List[Dict[str, float]]:
            ...
            coords.append(
                {
                    ...
                    "y": container_box["y"]
                    + (box[0] + box[2]) / 2 / 1000 * container_box["height"],
                    "box": box,
                }
            )
        return coords
