...
class PerformanceOptimizer:
    ...
    async def ultra_fast_solve(
        self, image_b64: str, captcha_type: str = "auto", require_consensus: bool = False
    ) -> SolveResult:
        """
        ...
        if result.confidence > 0.8:
            await self.cache.set_json(
                cache_key, {"solution": result.solution, "confidence": result.confidence}, ttl=3600
            )
    ...
    async def _parallel_race_solve(self, image_b64: str, captcha_type: str) -> SolveResult:
        """
        ...
        for completed_task in asyncio.as_completed(tasks):
            try:
                ...
                if result.confidence < 0.3:
                    logger.warning(
                        f"⚠️ Solver {result.solver_used} low confidence: {result.confidence}"
                    )
        ...
        if results:
            ...
            logger.info(
                f"⚠️ All solvers finished, picking best: {best.solver_used} ({best.confidence})"
            )
            return best

        # Complete failure
        logger.error(f"❌ All solvers failed. Errors: {errors}")
        return SolveResult(solution="", confidence=0.0, solver_used="none", time_ms=0)

    async def _parallel_consensus_solve(self, image_b64: str, captcha_type: str) -> SolveResult:
        """
        ...
        tasks = [
            self._timed_solve(name, func, image_b64, captcha_type) for name, func in strategies
        ]
        ...
        if winner_data["count"] >= 2:
            logger.info(
                f"🤝 [Consensus] Agreement found: {winner_sol} ({winner_data['count']} votes)"
            )
    ...
    async def _timed_solve(
        self, solver_name: str, solver_func, image_b64: str, captcha_type: str
    ) -> SolveResult:
        ...
        try:
            ...
            stats["count"] += 1
            stats["avg_time"] = (stats["avg_time"] * (stats["count"] - 1) + time_ms) / stats[
                "count"
            ]
            ...
            if "429" in error_str:
                error_code = 429
            elif "401" in error_str or "400" in error_str or "unauthorized" in error_str:
                error_code = 401
    ...
    async def _solve_with_gemini_flash(self, image_b64: str, captcha_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_code = (
                ...
                if any(x in str(e) for x in ["401", "400", "403"])
                else 500
            )
            await self.rotator.report_result("gemini", key, success=False, error_code=error_code)
            raise

    async def _solve_with_mistral(self, image_b64: str, captcha_type: str) -> Dict[str, Any]:
        ...
        try:
            result = await solver.solve_image(image_b64, captcha_type)
            return {"solution": result.solution, "confidence": result.confidence, "api_key": key}
        except Exception as e:
            error_code = (
                429
                if "429" in str(e)
    ...
    async def _solve_with_tesseract(self, image_b64: str, captcha_type: str) -> Dict[str, Any]:
        ...
        return {"solution": text, "confidence": confidence}

    def get_stats(self) -> Dict[str, Any]:
        """Get performance statistics"""
        cache_hit_rate = (self.stats["cache_hits"] / max(1, self.stats["total_solves"])) * 100
...
class APIKeyRotator:
    ...
    def _load_keys_from_env(self):
        ...
        for service in ["GEMINI", "MISTRAL", "OPENAI", "ANTHROPIC"]:
            ...
            if env_val:
                self.service_keys[service.lower()] = [
                    k.strip() for k in env_val.split(",") if k.strip()
                ]

        logger.info(
            f"🔑 Loaded keys for services: {list(self.service_keys.keys())} (Total keys: {sum(len(v) for v in self.service_keys.values())})"
        )
    ...
    async def report_result(
        self, service: str, key: str, success: bool, error_code: Optional[int] = None
    ):
        ...
        state = await self.redis.get_json(redis_key) or {
            "cooldown": 0,
            "success": 0,
            "fail": 0,
            "last_used": 0,
        ...
        if success:
            ...
            if error_code == 429:
                cooldown = 300  # Rate limit: 5 mins
            if error_code in [401, 403]:
                cooldown = 3600 * 24  # Invalid: 24h cooldown (until manual check)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            "status": "operational",
            "keys_tracked": sum(len(v) for v in self.service_keys.values()),
        }
