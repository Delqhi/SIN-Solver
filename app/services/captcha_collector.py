...
class CaptchaCollector:
    ...
    CAPTCHA_SOURCES = [
        ...
        # hCaptcha
        {"url": "https://accounts.hcaptcha.com/demo", "type": "hcaptcha", "provider": "hCaptcha"},
        # FunCaptcha (Arkose)
        {"url": "https://client-demo.arkoselabs.com/", "type": "funcaptcha", "provider": "Arkose"},
        ...
        # GeeTest
        {"url": "https://www.geetest.com/en/demo", "type": "geetest", "provider": "GeeTest"},
    ]
    ...
    async def collect_from_all_sources(self, max_per_source: int = 10) -> List[Dict[str, Any]]:
        """
        ...
        for source in self.CAPTCHA_SOURCES:
            ...
            try:
                captchas = await self.collect_from_url(
                    source["url"], expected_type=source["type"], max_count=max_per_source
                )
    ...
    async def collect_from_url(
        self, url: str, expected_type: str, max_count: int = 10
    ) -> List[Dict[str, Any]]:
        """
            ...
            for captcha in captchas:
                try:
                    ...
                    result = await session.execute(
                        select(CaptchaType).filter_by(name=captcha["type"])
                    )
    ...
    async def run_forever(self):
        ...
        while self.is_running:
            try:
                ...
                captchas = await self.collector.collect_from_all_sources(max_per_source=5)
