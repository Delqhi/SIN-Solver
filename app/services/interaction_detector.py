...
class UniversalInteractionDetector:
    ...
    async def detect_all(self, page: Page) -> Dict[str, Any]:
        ...
        try:
            screenshot = await page.screenshot(full_page=False, animations="disabled", type="jpeg", quality=85)
        ...
        report = self._perform_signal_fusion(
            dom_hits, net_hits, code_hits, vis_hits, dec_hits, 
            sky_hits, beh_hits, content_hits, log_hits, acc_hits,
            code_intel, struct_hits, semantic_hits, heat_hits
        )
        report["total_scan_ms"] = duration_ms
        report["screenshot_b64"] = screenshot_b64
        
        if report['found']:
            logger.info(f"🚨 [UID V3] {report['primary_type'].upper()} Identified via {report['primary_layer']} in {duration_ms}ms")
    ...
    async def _scan_visual_fragmented(self, screenshot: bytes, screenshot_b64: str) -> Dict[str, Any]:
        if not screenshot: return {"detections": [], "screenshot_b64": ""}
        try:
            ...
            async def solve_task(image_data, prompt, slice_info=None):
                ...
                try:
                    ...
                    sol = res.get("solution", "") if isinstance(res, dict) else getattr(res, "solution", "")
                    ...
                    if slice_info:
                        ...
                        for f in findings:
                            if "box_2d" in f:
                                b = f["box_2d"]
                                b[0] = int(b[0] * (mid_h if offset_y else mid_h) / 1000 + (mid_h if offset_y else 0))
                                b[1] = int(b[1] * (mid_w if offset_x else mid_w) / 1000 + (mid_w if offset_x else 0))
                                b[2] = int(b[2] * (mid_h if offset_y else mid_h) / 1000 + (mid_h if offset_y else 0))
                                b[3] = int(b[3] * (mid_w if offset_x else mid_w) / 1000 + (mid_w if offset_x else 0))
                    return findings
                finally:
            ...
            for i, s_data in enumerate(slices):
                tasks.append(solve_task(s_data, "Identify any interactive elements, buttons, or blockers in this sector. JSON: {'ui': [{'type': 'sector_hit', 'label': 'description', 'box_2d': [y,x,y,x]}]}", slice_info=slice_metadata[i]))
    ...
    async def _scan_skyvern_intent(self, screenshot_b64: str) -> Dict[str, Any]:
        if not screenshot_b64: return {}
        try:
            return await self.skyvern.analyze_image(screenshot_b64, task="Analyze blockade intent and determine strategy for bypass")
        except: return {}
    ...
    async def _scan_full_page_content(self, page: Page) -> List[Dict[str, str]]:
        try:
            ...
            findings = []
            solver_blocker_sigs = ["bot_detection", "fingerprintjs", "perimeterx", "datadome", "challenge-platform", "webdriver", "selenium", "playwright", "automation", "headless"]
            for sig in solver_blocker_sigs:
                if sig in content.lower():
                    findings.append({"type": "solver_blocker_marker", "value": sig, "source": "full_content"})
            return findings
        except: return []
    ...
    async def _scan_code_intelligence(self, page: Page) -> Dict[str, Any]:
        try:
            ...
            if "gate" in res.solution.lower():
                return {"type": "code_intelligence_gate", "confidence": 0.9, "details": res.solution}
            return {}
        except: return {}
    ...
    def _perform_signal_fusion(self, dom, net, code, vis, dec, sky, beh, content_hits=[], log_hits=[], acc_hits=[], code_intel={}, struct_hits=[], semantic_hits={}, heat_hits=[]) -> Dict[str, Any]:
        observations = []
        ...
        if code_intel.get("type"):
            observations.append({"type": code_intel["type"], "layer": "ai_intelligence", "confidence": code_intel.get("confidence", 0.85), "details": code_intel})
            
        for sh in struct_hits: observations.append({"type": sh["type"], "layer": "structure", "confidence": 0.88, "details": sh})
        for lh in log_hits: observations.append({"type": lh["type"], "layer": "logs", "confidence": 0.98, "details": lh})
        for ah in acc_hits: observations.append({"type": ah["type"], "layer": "accessibility", "confidence": ah.get("confidence", 0.9), "details": ah})
        for ch in content_hits: observations.append({"type": ch["type"], "layer": "content", "confidence": 0.82, "details": ch})

        traps = [d.get("details", {}) for d in dec if d.get("type") == "honeypot"]
        for d in dec: observations.append({"type": d.get("type", "deception"), "layer": "deception", "confidence": d.get("confidence", 0.94), "details": d})

        for d_type, el in dom:
            is_trap = False
            if hasattr(el, 'id') and any(t.get('id') == el.id for t in traps): is_trap = True
            if not is_trap: observations.append({"type": d_type, "layer": "dom", "confidence": 0.96, "element": el})

        for n_type in net: observations.append({"type": n_type, "layer": "network", "confidence": 0.99})
        for c in code: observations.append({"type": c["type"], "layer": "code", "confidence": 0.92})
        for b in beh: observations.append({"type": b.get("type", "behavior"), "layer": "behavior", "confidence": 0.85})
        ...
        for h in heat_hits:
            observations.append({"type": "interactive_heat", "layer": "heat", "confidence": 0.65, "box": h["box"], "label": h["tag"]})
        ...
        if any(x in pt for x in ["captcha", "cloudflare", "hcaptcha", "recaptcha", "turnstile", "aws_waf", "datadome"]):
            resolver = "steel_precision"
        elif any(x in pt for x in ["form", "login", "modal", "overlay", "honeypot", "gate"]):
            resolver = "stagehand"
        elif primary["layer"] == "skyvern" or primary.get("is_blockade") or primary["layer"] == "semantic":
        ...
        resolver = "agent_zero"
        pt = str(primary["type"]).lower()
        if any(x in pt for x in ["captcha", "cloudflare", "hcaptcha", "recaptcha", "turnstile", "turnstile"]):
            resolver = "steel_precision"
        elif any(x in pt for x in ["form", "login", "modal", "overlay", "honeypot", "gate"]):
            resolver = "stagehand"
