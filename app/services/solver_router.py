...
class RemoteSolverRouter:
    ...
    async def solve_image(self, image_base64: str, captcha_type: str = "auto", use_cache: bool = True) -> UnifiedSolution:
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code != 200:
                    ...
                    return UnifiedSolution(solution="", confidence=0.0, time_ms=0, solver_used="remote", cost=0.0, error=f"API Error: {response.status_code}")
                
                data = response.json()
                return UnifiedSolution(**data)
        except Exception as e:
            logger.error(f"Remote solver connection error: {e}")
            return UnifiedSolution(solution="", confidence=0.0, time_ms=0, solver_used="remote", cost=0.0, error=str(e))
    ...
    async def solve_auto(self, image_base64: str = "", question: str = "", use_cache: bool = True) -> UnifiedSolution:
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(
...
class SolverRouter:
    ...
    def __init__(self):
        ...
        self.stats = {
            ...
            "mistral_success": 0, "gemini_success": 0,
            "groq_success": 0, "yolo_success": 0,
            "human_success": 0,
            "total_fallbacks": 0,
            "total_time_ms": 0, "total_cost": 0.0,
            "fuzzy_corrections": 0
        }
    
    async def _get_solvers(self):
        if self.mistral is None: self.mistral = await get_mistral_solver()
        return self.mistral
    ...
    async def solve_image(self, image_base64: str, captcha_type: str = "auto", use_cache: bool = True, instruction: Optional[str] = None) -> UnifiedSolution:
        ...
        if use_cache:
            ...
            if cached:
                ...
                self.analytics.record_solve(True, solve_time_ms, "cache", captcha_type, from_cache=True)
                return UnifiedSolution(solution=cached["solution"], confidence=cached["confidence"], 
                                       time_ms=solve_time_ms, solver_used="cache", cost=0.0, cache_hit=True)
        ...
        try:
            tasks = [
                ...
                self.groq.solve_image(image_base64, captcha_type) if settings.groq_api_key else None,
            ...
            if not all_results:
                return UnifiedSolution(solution="", confidence=0.0, time_ms=0, solver_used="none", cost=0.0, error="All solvers failed in pool")
            ...
            for sol, count in counts.most_common():
                ...
                if count >= 2:
                    matching_results = [r for r in all_results if r["solution"].lower().strip() == sol]
            ...
            if not winner:
                ...
                if best_single["confidence"] < 0.85:
                    logger.info("🆘 [RECOVERY] Confidence low and no consensus. Engaging High-Tier Gemini...")
                    high_tier_res = await self.gemini.solve_image(image_base64, captcha_type, high_tier=True)
                    if high_tier_res.get("success") and high_tier_res.get("confidence", 0) > 0.9:
                        ...
                        winner = best_single
                        logger.info(f"🏆 [CONSENSUS] Recovery failed or low confidence, selecting best single: {winner['solver']} ({winner['confidence']})")
                else:
                    winner = best_single
                    logger.info(f"🏆 [CONSENSUS] No consensus, selecting best single: {winner['solver']} ({winner['confidence']})")
            else:
                logger.info(f"✅ [CONSENSUS] Final Decision: {winner['solution']} via {winner['solver']}")

            if winner:
                solve_time_ms = int((time.time()-start_time)*1000)
                self.stats[f"{winner['solver'].split('-')[0]}_success" if '-' in winner['solver'] else "mistral_success"] += 1
                self.analytics.record_solve(True, solve_time_ms, winner['solver'], captcha_type)
                
                # Cache successful solution
                if use_cache:
                    cache_key = f"captcha:image:{self._compute_image_hash(image_base64)}"
                    await self.cache.set_json(cache_key, {"solution": winner["solution"], "confidence": winner["confidence"]}, ttl=3600)
        ...
        return UnifiedSolution(solution="", confidence=0.0, time_ms=0, solver_used="none", cost=0.0, error="All solvers failed")
    ...
    async def solve_auto(self, image_base64: str, question: Optional[str] = None, use_cache: bool = True) -> UnifiedSolution:
        if image_base64: return await self.solve_image(image_base64, "auto", use_cache)
        if question: return await self.solve_text(question, use_cache)
        return UnifiedSolution(solution="", confidence=0.0, time_ms=0, solver_used="none", cost=0.0, error="No input")

    async def get_stats(self) -> Dict[str, Any]:
        return self.stats

    async def health_check(self) -> Dict[str, Any]:
        return {"status": "healthy", "solvers": ["gemini", "mistral", "groq", "yolo"], "paid_services": False}
