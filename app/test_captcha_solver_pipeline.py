...
class TestOCREngines(unittest.TestCase):
    ...
    def test_paddleocr_available(self):
        ...
        self.assertTrue(
            self.paddleocr.available,
            "PaddleOCR should be installed"
        )
    ...
    def test_paddleocr_text_extraction(self):
        ...
        # Average confidence
        avg_conf = np.mean([r for r in results if r > 0.0]) if any(r > 0.0 for r in results) else 0.0
...
class TestPipelineIntegration(unittest.TestCase):
    ...
    def test_solve_text_captcha(self):
        ...
        for img_path in test_images[:3]:
            ...
            logger.info(f"Solved {img_path.name}: {result['answer']} (conf: {result['confidence']:.2f})")
...
class TestPerformanceMetrics(unittest.TestCase):
    ...
    def test_paddleocr_accuracy_estimate(self):
        ...
        avg_conf = np.mean([c for c in confidences if c > 0.0]) if any(c > 0.0 for c in confidences) else 0.0
        logger.info(f"PaddleOCR average confidence: {avg_conf:.2f}")
