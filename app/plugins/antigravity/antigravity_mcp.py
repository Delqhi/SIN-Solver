...
class AntigravityMCP:
    ...
    async def _handle_add_account(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                return {"success": True, "message": f"Account {account_id} added successfully"}
            else:
                return {"success": False, "error": "Failed to validate account credentials"}
        except Exception as e:
    ...
    async def _handle_remove_account(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if antigravity_connector.remove_account(account_id):
                return {"success": True, "message": f"Account {account_id} removed successfully"}
            else:
    ...
    async def _handle_switch_account(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if antigravity_connector.switch_account(account_id):
                return {"success": True, "message": f"Switched to account {account_id}"}
            else:
                return {"success": False, "error": f"Account {account_id} not found"}
        except Exception as e:
        ...
        try:
            ...
            async with stdio_server() as (read_stream, write_stream):
                await self.server.run(
                    read_stream=read_stream, write_stream=write_stream, notification_callback=None
                )
