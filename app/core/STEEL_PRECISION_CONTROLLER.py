...
from app.services.vision_orchestrator import (
    RateLimitError,
    extract_click_coordinates_from_response
)
...
class SteelPrecisionController:
    ...
    async def initialize(self, stealth_mode: bool = True, use_local_browser: bool = False, session_id: Optional[str] = None):
        """
        ...
        for attempt in range(max_retries):
            try:
                if use_local_browser:
                    ...
                    self.browser = await self._playwright.chromium.launch(headless=False, args=["--disable-blink-features=AutomationControlled"])
                else:
                    logger.info(f"🔗 Connecting to Steel CDP: {self.steel_endpoint} - Attempt {attempt+1}")
                    self.browser = await self._playwright.chromium.connect_over_cdp(self.steel_endpoint)
                break
        ...
        try:
            ...
            if proxy:
                logger.info(f"🌐 [CEO] Tunneling through proxy: {proxy} ({proxy_metadata.get('countryCode', '??')})")

            self.context = await self.browser.new_context(
                viewport={"width": 1920, "height": 1080},
                user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
                storage_state=storage_state,
                proxy={"server": proxy} if proxy and not use_local_browser else None # Use proxy if not local
            )
            ...
            if stealth_mode:
                ...
                await self.stealth_engine.apply_ultimate_stealth(self.page, proxy_metadata=proxy_metadata)
                logger.info(f"🛡️ CEO-GRADE Ultimate Stealth active (GeoSync: {proxy_metadata.get('timezone', 'UTC')})")
            else:
    ...
    async def get_precise_screenshot(self) -> bytes:
        try:
            ...
            result = subprocess.run(["agent-browser", "screenshot", temp_path], capture_output=True, timeout=5)
        ...
        if not self.page: return b""
        return await self.page.screenshot(type="png", full_page=False, animations="disabled")

    async def get_vercel_snapshot(self) -> dict:
        try:
            result = subprocess.run(["agent-browser", "snapshot", "-i", "--json"], capture_output=True, text=True, timeout=10)
    ...
    async def vercel_click(self, ref: str) -> bool:
        try:
            result = subprocess.run(["agent-browser", "click", f"@{ref}"], capture_output=True, timeout=5)
    ...
    async def detect_silent_captcha(self) -> str:
        ...
        silent_detectors = {
            "cloudflare": ["/cdn-cgi/challenge-platform/h/g/orchestrate", "challenges.cloudflare.com"],
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if primary_type in ["cloudflare", "recaptcha"]:
                    logger.info(f"⚖️ [CEO] Optimizing bypass for {primary_type} score-based challenge...")
                    ...
                    if primary_type == "cloudflare":
                        ...
                        await asyncio.sleep(5)
                        if "challenge" not in self.page.url: return True

                deceptions = [f for f in detection_report.get("all_observations", []) if f.get("layer") == "deception"]
                if deceptions:
                    logger.warning(f"🚨 [CEO] Anti-Bot Deception detected: {[d['type'] for d in deceptions]}. Hardening stealth...")
                ...
                logger.info(f"🚨 [CEO] Detected Blockade: {primary_type} -> Handing over to {target_agent.upper()}")
                ...
                if target_agent == "skyvern":
                    ...
                    if primary_type in ["text_input", "text", "math"]:
                        ...
                        if solution and getattr(solution, 'solution', None):
                            await HumanBehavior.human_type(self.page, solution.solution, element_selector='input:not([type="hidden"])')
                        ...
                        for f in detection_report.get("all_observations", []):
                            if "box" in f and f["box"]:
                                ...
                                x = (box[1] + box[3]) / 2 / 1000 * (viewport['width'] if viewport else 1920)
                                y = (box[0] + box[2]) / 2 / 1000 * (viewport['height'] if viewport else 1080)
    ...
    async def _solve_with_skyvern(self, url: str) -> bool:
        ...
        try:
            ...
            result = await skyvern.navigate_and_solve(url, task="Bypass any gate, captcha, or blockade and reach the content.")
