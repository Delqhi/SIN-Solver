...
class Conductor:
    ...
    def _init_redis(self):
        ...
        try:
            ...
            logger.warning(
                f"⚠️  Failed to connect to room-04-redis-cache, falling back to localhost: {e}"
            )
            try:
                self.redis_client = redis.Redis(
                    host="localhost",
    ...
    def compose_symphony(self, name: str) -> Symphony:
        symphony = Symphony(id=str(uuid.uuid4()), name=name)
    ...
    def add_track_to_symphony(self, symphony_id: str, track_name: str, priority: int = 1) -> Track:
        ...
        track = Track(id=str(uuid.uuid4()), name=track_name, priority=priority)
        symphony.tracks.append(track)
        self._save_symphony(symphony)
        return track

    def add_movement_to_track(
        self, symphony_id: str, track_id: str, movement_name: str, parallel: bool = False
    ) -> Movement:
        symphony = self._get_symphony(symphony_id)
        if not symphony:
            raise ValueError(f"Symphony {symphony_id} not found")
        ...
        movement = Movement(id=str(uuid.uuid4()), name=movement_name, parallel=parallel)
        track.movements.append(movement)
        self._save_symphony(symphony)
        return movement

    def add_shard_to_movement(
        self, symphony_id: str, track_id: str, movement_id: str, shard_name: str, payload: Dict
    ) -> Shard:
        symphony = self._get_symphony(symphony_id)
        if not symphony:
            raise ValueError("Symphony not found")
        ...
        shard = Shard(id=str(uuid.uuid4()), name=shard_name, payload=payload)
