...
class AcademicBenchmarkRunner:
    ...
    async def run_mca_bench(self):
        ...
        for case in os.listdir(path):
            ...
            try:
                ...
                response = await ViPerStrikeProcessor.process(
                    self.solver, image_base64, metadata["task"]
                )
            ...
            for file in files:
                if file.endswith((".png", ".jpg", ".jpeg")):
                    ...
                    try:
                        # Use unified ViPerStrikeProcessor
                        response = await ViPerStrikeProcessor.process(
                            self.solver, image_base64, metadata.get("task", "Solve this CAPTCHA")
                        )
    ...
    def _verify_solution(self, response: Dict, targets: List) -> bool:
        ...
        if not detected_boxes and not targets:
            return True
        if not detected_boxes or not targets:
            return False
